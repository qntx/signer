...
- **Full cross-implementation KAT suite.** Every chain crate's `tests.rs` is now a protocol-equivalence check: the expected address, digest, and signature bytes are generated by the independent `@noble/curves` + `@noble/hashes` + `@scure/base` JavaScript stack (see `tests/goldens/generate.mjs`) and pasted in as inline `const` literals. A test that fails is a real drift from the external reference, not a mismatch with the signer's own past output.
- **Cross-cutting sanity checks consolidated into `signer_primitives::tests`.** Constructor validation, curve-order rejection, `verify_prehash_*` dispatch, `SignOutput` wire layout, and `Debug [REDACTED]` are exercised once in the primitive crate. Per-chain tests no longer duplicate them — every test in every chain crate now pins a single, chain-specific piece of the signing contract.
- Removed `~300` lines of redundant `from_bytes_roundtrip` / `rejects_invalid_input` / `deterministic_signature` tests that restated primitive behaviour.
